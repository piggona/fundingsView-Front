<template>
  <span>
      <span class="" id="titlesearch-form-group">
      ...
      <div v-if="isSearch">
        <a-input
          ...
          class="form-control"
        />
        <a-button @click="onDataPut">Put</a-button>
      </div>
      <template 
        v-for="item in searchItems"
      >
        <a-tag :key="item" closable @close="() => handleClose(item)">{{ item }}</a-tag>
      </template>
  </span>
</template>
...
<script>
export default {
  props: ["id", "type", "data", "isSearch","template"],
  data() {
    return {
      checked: true,
      value: "and",
  ...
  methods: {
    handleClose(removedTag) {
      const tags = this.searchItems.filter(tag => tag !== removedTag);
      console.log(tags);
      this.searchItems = tags;
      this.onInputChange()
    },
    onDataPut() {
